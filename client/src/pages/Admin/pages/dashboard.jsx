...
function Dashboard(props) {
    ...
    useEffect(_ => {
        if (searchType === "patient") {
            API.getAllPatients((nameSearch === "") ? null : {
                "$expr": {
                    "$regexMatch": {
            ...
            API.getAllVolunteers((nameSearch === "") ? null : (nameSearch === "") ? null : {
                "$expr": {
                    "$regexMatch": {
                        "input": { "$concat": ["$first name", " ", "$last name"] },
                        "regex": nameSearch,  //Your text search here
    ...
    function onChangeData(data, type) {
        if (type === 'patient') {
            devConsole.log('mod patient',data)
            API.updatePatient(data).then(res=>
                devConsole.log("success!")
            ).catch(console.err)
        }
        else if (type === 'volunteer') {
            devConsole.log('mod volunteer')
        }
    }
    return (
        <div className="background">
            ...
            <div className="content">
                ...
                <div className="doc">
                    <PDFFromDataVolunteerPatient data={selected} onChange={onChangeData} />
                </div>
            </div>
        </div>
    )
